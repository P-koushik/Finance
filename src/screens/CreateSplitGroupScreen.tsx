...
import { ArrowLeft, Check, Plus, Scale } from 'lucide-react-native';
...
const avatarColors = ['#6AAA6B', '#C96552', '#D1A51E', '#D1A51E', '#378260'];
...
export function CreateSplitGroupScreen() {
  ...
  const createSplitGroup = useMutation({
    mutationFn: async () => {
      const splitGroup = await financeApi.createSplitGroup({
        name: name.trim(),
        description: '',
        category: 'friends',
      ...
      await Promise.all(
        Array.from(selectedIds).map(userId =>
          financeApi.addSplitGroupMember(splitGroup.id, userId),
        ),
      );

      if (parsedAmount > 0) {
        await financeApi.createSplitExpense(splitGroup.id, {
          title: name.trim(),
    ...
    onError: () => Alert.alert('Split', 'Could not create split.'),
  });

  const canCreate =
    Boolean(name.trim()) &&
    Boolean(currentUserId) &&
    (parsedAmount <= 0 || selectedIds.size > 0);
  ...
  return (
    <SafeAreaView edges={['top']} className="flex-1 bg-[#EEF4EE]">
      ...
      <View className="flex-row items-center gap-3 px-5 pb-3 pt-3">
        ...
        <Text className="text-[20px] font-black text-[#24352E]">New split</Text>
      </View>
      ...
      >
        <View className="mb-[22px] items-center">
          <View className="h-[76px] w-[76px] items-center justify-center rounded-[24px] bg-[#EAF2EA]">
            <Scale color="#2E5D4B" size={34} strokeWidth={2.4} />
          </View>
        </View>
        ...
        <Text className="mb-2 mt-5 text-[13px] font-black text-[#6E9081]">
        ...
        <View className="gap-3">
          {usersQuery.isLoading || profileQuery.isLoading ? (
