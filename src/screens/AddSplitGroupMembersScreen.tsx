...
export function AddSplitGroupMembersScreen() {
  ...
  const addMember = useMutation({
    mutationFn: (user: UserSearchResult) =>
      financeApi.addSplitGroupMember(splitGroupId, user.id),
    onSuccess: async () => {
  ...
  return (
    <SafeAreaView edges={['top']} className="flex-1 bg-[#EEF4EE]">
      ...
      <Text className="px-5 text-[13.5px] font-bold text-[#8D9B93]">
        to {splitGroupQuery.data?.name ?? 'split'}
      </Text>
      ...
      <View className="absolute bottom-4 left-5 right-5">
        <Pressable
          ...
          onPress={() =>
            navigation.navigate('SplitGroupDetails', { splitGroupId })
          }
        >
